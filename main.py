...
class CalorieApp(tk.Tk):
    def __init__(self):
        ...
        self.total_protein = 0  # Accumulate protein
        self.total_carbs = 0  # Accumulate carbs
        self.total_fat = 0  # # Accumulate fat

    def update_macros_display(self):
        "Update the macro nutrient display in the GUI."
        self.carbs_label.config(text=f"Carbs: {self.total_carbs:.1f}g")
        self.protein_label.config(text=f"Protein: {self.total_protein:.1f}g")
        self.fat_label.config(text=f"Fat: {self.total_fat:.1f}g")
    ...
    def search_food(self, meal):
        ...
        if calories > 0 or protein > 0 or carbs > 0 or fat > 0: # did we get any non-0 data?
            # show calorie info
           ...
           # Accumulate and update macronutrients
